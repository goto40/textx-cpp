...
namespace {
    ...
    ta::Pattern transform_match2pattern(ParseState parsestate, METAMODEL &mm, RULE& rule, std::shared_ptr<const ta::Match> match);

    struct Eolterm{};
    struct None{};
    using Repeat_modifiers = std::variant<ta::Pattern, Eolterm, None>;
    Repeat_modifiers get_repeat_modifiers(ParseState parsestate, METAMODEL &mm, RULE& rule, std::shared_ptr<const ta::Match> m);

    /** get the repat modifiers info from a Match from ta::optional(ref("repeat_modifiers")) */
    Repeat_modifiers extract_repeat_modifiers(ParseState parsestate, METAMODEL &mm, RULE& rule, std::shared_ptr<const ta::Match> repeat_modifiers_match) {
    ...
    ta::Pattern normal_expression_or_unordered_choice(ParseState parsestate, METAMODEL &mm, RULE& rule, std::shared_ptr<const ta::Match> match, bool use_choice, const Repeat_modifiers &repeat_modifiers) {
        auto &expr = *match;
    ...
    textx::AttributeCardinality get_multiplicity(std::shared_ptr<const textx::arpeggio::Match> match) {
    ...
    std::unordered_map<
        std::string,
        std::function<
            ta::Pattern(ParseState parsestate, METAMODEL &mm, RULE& rule, std::shared_ptr<const ta::Match> match)
        >
    > transform_match2pattern_map = {
        {
            "rule://textx_rule_body",
            [](ParseState parsestate, METAMODEL &mm, RULE& rule, std::shared_ptr<const ta::Match> match) -> ta::Pattern {
...
namespace textx {
    ...
    Rule::Rule(const textx::Metamodel& mm, std::string_view name, std::shared_ptr<const ta::Match> rule_params, std::shared_ptr<const ta::Match> rule_body) {
        // << rule_body << "\n";
        auto& rule = *this;
        rule.name = name;
        ...
        if (rule_params->children.size()>0) {
            ...
            auto add_param=[&](std::shared_ptr<const ta::Match> p) {
    ...
    void Rule::post_process_created_rule(textx::Metamodel& mm, std::string_view name, std::shared_ptr<const ta::Match> rule_params, std::shared_ptr<const ta::Match> rule_body) {
        auto& rule = *this;
    ...
    textx::AttributeCardinality Rule::get_attribute_cardinality(std::shared_ptr<const textx::arpeggio::Match> match, std::string name) {
        TEXTX_ASSERT(attribute_info.count(name)>0);
        bool is_boolean_involved = false;
        std::function<size_t(std::shared_ptr<const textx::arpeggio::Match>,size_t)> traverse;
        traverse = [&is_boolean_involved,&name,&traverse](std::shared_ptr<const textx::arpeggio::Match> m,size_t c) -> size_t {
    ...
    void Rule::determine_rule_type_and_adjust_inh_by(const textx::Metamodel& mm) {
        ...
        std::function<bool(std::shared_ptr<const textx::arpeggio::Match>)> add_inh_by_rule;
        add_inh_by_rule = [&,this](std::shared_ptr<const textx::arpeggio::Match> node) -> bool {
